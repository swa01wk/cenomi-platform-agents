...
def is_locked(locked_fields: Dict[str, List[str]], agent_id: str, key: str) -> bool:
    return key in (locked_fields.get(agent_id) or [])
...
def guess_array_field_key(
    cfg: Dict[str, Any], stage_required: List[str]
) -> Optional[str]:
...
def choose_issue_bundle(
    issues: List[Dict[str, Any]], max_items: int = 4
) -> List[Dict[str, Any]]:
...
def ask_for_issues(cfg, issues, stage_id, draft, just_switched=False):
    # First message after agent switch → no "thanks"
    if just_switched and (not draft or not any(draft.values())):
        missing = [i for i in issues if i["kind"] == "missing"]
        labels = ", ".join([friendly_label(cfg, i["key"]) for i in missing[:4]])

        return (
            f"Got it — I’ll help you with **{cfg.get('name')}**.\n"
            f"To get started, I’ll need: **{labels}**.\n"
            "You can share everything in one message (or whatever you know)."
        )
    ...
    llm = ChatOpenAI(model=cfg.get("model"), temperature=0.6)
    out = llm.invoke([
        SystemMessage(
            content=(
                "You are a friendly assistant helping collect service details. "
                "Be natural and concise. Ask for missing items together. "
                "Do NOT say 'thanks for sharing' unless user actually provided data."
            )
        ),
        HumanMessage(
            content=f"""
...
def validate_draft(
    cfg: Dict[str, Any], stage_id: str, draft: Dict[str, Any]
) -> List[Dict[str, Any]]:
    """
    ...
    for k, f in spec_by_key.items():
        ...
        if t == "number":
            try:
                ...
                issues.append(
                    {"key": k, "kind": "invalid", "message": "Please provide a number."}
                )
                continue
...
@task
def extract_fields(
    cfg: Dict[str, Any], user_text: str, draft: Dict[str, Any]
) -> Dict[str, Any]:
    ExtractionModel = build_extraction_model(cfg)
    llm = ChatOpenAI(
        model=cfg.get("model") or MODEL_DEFAULT, temperature=0
    ...
    sys = (
        (
            ...
            or "You are a helpful assistant collecting details conversationally."
        )
        + "\n\n"
...
def run_subagent(inputs: dict) -> dict:
    ...
    """
    just_switched = bool(inputs.get("just_switched_agent"))
    ...
    if issues_all:
        ...
        q = ask_for_issues(cfg, bundle, stage_id, draft, just_switched)
    ...
    for tool_name in st.get("required_tools") or []:
        ...
        # ✅ normalize: tools expect `documents`
        if "documents" not in tool_payload:
            if "attachments" in tool_payload:
                tool_payload["documents"] = tool_payload["attachments"]
            elif attachments:
                tool_payload["documents"] = attachments

        result = tool_runner(tool_name, tool_payload)
    ...
    if nxt:
        ...
        if nxt_issues:
            ...
            q = (
                f"Great — I’ve captured the initial details. "
                f"Next I need a bit more:\n"
                f"{ask_for_issues(cfg, bundle, stage_id, draft, just_switched)}"
            )
