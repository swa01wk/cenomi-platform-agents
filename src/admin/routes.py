    ...
    for st in payload.stages:
        for rf in st.required_fields:
            if rf not in field_keys:
                raise HTTPException(
                    status_code=400,
                    detail=f"Stage '{st.stage_id}' references unknown field '{rf}'",
                )
    try:
...
def patch_agent(agent_id: str, payload: AgentUpdate):
    try:
        return store.update_agent(agent_id, payload.model_dump(exclude_unset=True))
